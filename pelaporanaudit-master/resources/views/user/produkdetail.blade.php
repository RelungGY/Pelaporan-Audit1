    ...
    <div class="row">
        <div class="col-md-6">
            ...
            <div class="d-flex mt-2 thumbnail-nav">
                @if ($product->images && is_array(json_decode($product->images)))
                    @foreach (json_decode($product->images) as $key => $image)
                        <div class="thumbnail-item mx-1" onclick="$('#carouselExampleIndicators{{ $product->id }}').carousel({{ $key }})">
        ...
        <div class="col-md-6">
            ...
            <div class="product-details">
                <div class="ratings mb-2">
                    <span class="rating-score">{{ $product->rating ?? '4.9' }}</span>
                    <div class="stars">
                        @for($i = 0; $i < 5; $i++)
                            <i class="fas fa-star text-warning"></i>
                        @endfor
                    </div>
                    <span class="reviews-count">{{ $product->reviews_count ?? '2,3RB' }} Penilaian</span>
                    <span class="sales-count">{{ $product->sales_count ?? '4,5RB' }} Terjual</span>
                </div>

                <div class="price-section mb-3">
                    <h3 class="text-danger mb-0" data-price="{{ $product->price_per_day }}">
                        Rp{{ number_format($product->price_per_day, 0, ',', '.') }}
                    </h3>
                    @if($product->original_price)
                        <small class="text-muted text-decoration-line-through">Rp{{ number_format($product->original_price, 0, ',', '.') }}</small>
                    @endif
                </div>
                ...
                <div class="total-price-section mb-3">
                    <h6>Total Harga</h6>
                    <div class="input-group" style="width: 150px;">
                        <input type="text" class="form-control text-center total-price" value="Rp{{ number_format($product->price_per_day, 0, ',', '.') }}" readonly>
                    </div>
                </div>
